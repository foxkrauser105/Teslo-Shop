...
export const CustomHeader = () => {
  ...
  const query = searchParams.get(URLParamKeysTypes.Query) || "";

  const handleSearchInputChange = (event: KeyboardEvent<HTMLInputElement>) => {
    ...
    if (StringUtils.StringIsNullEmptyOrWhiteSpace(query)){
      newSearchParams.delete(URLParamKeysTypes.Query)
    }
    else {
      newSearchParams.set(URLParamKeysTypes.Query, query!)
    }
            ...
            <div className="hidden md:flex items-center space-x-2">
              <div className="relative">
                ...
                <Input 
                  ...
                  onKeyDown={(event) => handleSearchInputChange(event)}
                  defaultValue={query}
                />
              </div>
            </div>
